...
export default function TenantRoles() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [rolesRes, permsRes] = await Promise.all([
        ...
        api.get(`/roles/${tenantSlug}/permissions`)
      ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingRoleId) {
        await api.put(`/roles/${tenantSlug}/${editingRoleId}`, {
          name: roleName,
          permissions: selectedPermissions
        });
      } else {
        await api.post(`/roles/${tenantSlug}`, {
          name: roleName,
          permissions: selectedPermissions
        });
      }
  ...
  const handleDelete = async (role: Role) => {
    if (role.name === 'Administrador') {
      alert('O cargo Administrador não pode ser excluído.');
      return;
    }
    
    if (window.confirm(`Tem certeza que deseja excluir o cargo ${role.name}?`)) {
      try {
        await api.delete(`/roles/${tenantSlug}/${role.id}`);
        await fetchData();
      } catch (err: any) {
    ...
    <div className="page-container">
      <header className="page-header">
        <div>
          ...
          <p className="page-subtitle">Configure os níveis de acesso para a equipe Tenant</p>
        </div>
        {hasPermission('TenantRoles:Create') && (
              ...
              <tr key={role.id}>
                ...
                <td className="text-right">
                  <div className="flex justify-end gap-3">
                    {hasPermission('TenantRoles:Edit') && (
                      <button 
                    ...
                    {hasPermission('TenantRoles:Delete') && (
                      <button 
                        onClick={() => handleDelete(role)}
                        className="btn-icon text-red-400 hover:text-red-300 hover:bg-red-900/30"
                        title="Excluir"
                        disabled={role.name === 'Administrador'}
                        style={{ opacity: role.name === 'Administrador' ? 0.3 : 1, cursor: role.name === 'Administrador' ? 'not-allowed' : 'pointer' }}
                      >
                        <Trash2 size={18} />
                      </button>
                    )}
              ...
              <div className="modal-body" style={{ paddingTop: '0.75rem' }}>
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    disabled={roleName === 'Administrador'}
                    required
                  />
                </div>
                  ...
                  <div className="permissions-groups">
                    {Object.entries(
                      ...
                      const groupNames: Record<string, string> = {
                        ...
                        TenantTeam: 'Usuários da Equipe',
                        TenantRoles: 'Cargos e Permissões',
